...
for j in trange(10):
    ...
    print("label*****: ",np.asarray(ori_label).max(),np.asarray(gen_label).max())
    #concatenate
    ...
    for i in range(proba.shape[0]):
        # cur_proba = non_linear_model.predict_proba(X_test[i])
        # print(cur_proba.shape)
...
score["kl"] = klscore
score["is"] = isscore
pickle.dump(score,open("1_10_score","wb"))
